...
import numpy as np
import os
from .utils import tempfile_from_url
from tempfile import NamedTemporaryFile
from .close_packings_via_layers import close_packing

class example_data_isaacs:
     def __init__(self,url,coord_type,mode,offline_fp=None):
          self.url=url
          self.coord_type=coord_type
          if mode == 'online':
               with tempfile_from_url(url) as fp:
          ...
          self.original=np.loadtxt(path,skiprows=2,usecols=(1,2,3),unpack=False) # store the original coordinates
          if mode == 'online':
               os.remove(path)
     def __len__(self):
          return len(self.original)

class fcc_au_cart(example_data_isaacs,close_packing):
     def __init__(self,mode='offline'):
          url="http://isaacs.sourceforge.net/tests/au-cart.xyz"
          dir_name=os.path.dirname(os.path.abspath(__file__))
          path=os.path.join(dir_name, "isaacs_data", "au-cart.xyz") # the last line is deleted from the downloaded original file
          super().__init__(url,coord_type='cartesian',mode=mode,offline_fp=path)
          self.cartesian=self.original # the cartesian coordinates is the same as the original coordinates
          self.normalized=2*self.cartesian/2.877925 # rescale to make the radius of the atom 1.0
...
class hcp_ru_frac(example_data_isaacs,close_packing):
     def __init__(self,mode='offline'):
          url="http://isaacs.sourceforge.net/tests/ru-frac.xyz"
          dir_name=os.path.dirname(os.path.abspath(__file__))
          path=os.path.join(dir_name, "isaacs_data", "ru-frac.xyz") # the last line is deleted from the downloaded original file
          super().__init__(url,coord_type='cartesian',mode=mode,offline_fp=path)
          self.fractional=self.original # the fractional coordinates is the same as the original coordinates
          ...
          self.normalized=2*self.cartesian/5.29987615 # rescale to make the radius of the atom 1.0
          self.data=self.normalized # set self.data to be the normalized coordinates
          # now methods in close_packing can be used on self.data

class fcc_si_frac(example_data_isaacs,close_packing):
     def __init__(self,mode='offline'):
          url="http://isaacs.sourceforge.net/tests/si-frac.xyz"
          dir_name=os.path.dirname(os.path.abspath(__file__))
          path=os.path.join(dir_name, "isaacs_data", "si-frac.xyz") # the last line is deleted from the downloaded original file
          super().__init__(url,coord_type='cartesian',mode=mode,offline_fp=path)
          self.fractional=self.original # the fractional coordinates is the same as the original coordinates
          # Transofrm the fractional coordinates to cartesian coordinates
          # parameters from the website
          ...
          M  = np.array([[a,0,0],
               ...
               [0,0,c]])
          self.cartesian=self.fractional@M
          self.normalized=2*self.cartesian/4.6939 # rescale to make the radius of the atom 1
          self.data=self.normalized # set self.data to be the normalized coordinates
          # now methods in close_packing can be used on self.data