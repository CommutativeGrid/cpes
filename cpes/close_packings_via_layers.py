...
class points_3d:
    ...
    def plot(self, alpha=0.8, size=400):
        ...
        ax.scatter(self.data[:, 0], self.data[:, 1],
                   self.data[:, 2], alpha=alpha, s=size, c=color_vector)
    ...
    def diameter(self):
        ...
        if hasattr(self, '_diameter'):
            ...
            self._diameter = max(distance.cdist(
                self.data, self.data, 'euclidean').flatten())
            return self._diameter
    ...
    def add_perturbtation(self):
        ...
        flattened_perturbed = flattened + \
            np.random.normal(mean, sigma, len(flattened))
        ...
        if style == 'homcloud':
            if save:
                ...
                if os.path.isfile(file_path):
                    ...
                    np.savetxt(file_path,sorted_result,fmt=['%d']+['%.6f']*3,delimiter=' ')
