...
class simple_cubic(crystal_system):
        ...
        for x in range(self.length):
            for y in range(self.length):
                for z in range(self.length):
                    self._unit_grid.append([x,y,z]) # append atomic coordinates at each integer valued points
        self.data=np.array(self._unit_grid)*self.side_l # convert list to numpy array
...
class body_centered_cubic(simple_cubic):
    """bcc class"""
    def __init__(self,num_cells_axis=10,side_l=1.0):
        self.side_l=side_l
        ...
        for x in range(0,self.length-1):
            for y in range(0,self.length-1):
                for z in range(0,self.length-1):
                    self._unit_grid.append([x+0.5,y+0.5,z+0.5]) # add an atom at the body center of each conventional cell
        self.data=np.array(self._unit_grid)*self.side_l # convert list to numpy array
